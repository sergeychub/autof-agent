...
```bash
ls -l /dev/usb/lp*
for device in /sys/class/usbmisc/lp*; do
  printf '%s: ' "$(basename "$device")"
  cat "$device/device/../serial"
done
```

Для USB-принтера рекомендуется указать серийный номер, а не изменяемый номер `lpN`:
...
Перед каждым заданием агент сопоставляет `deviceSerial` с актуальным `/dev/usb/lpN` через Linux sysfs. Это защищает от смены номера после перезагрузки или подключения второго принтера. Если заданы оба поля, `deviceSerial` имеет приоритет и агент не отправит данные в запасной путь при несовпадении серийного номера.

Установщик добавляет пользователя агента в группу `lp`. После изменения групп перезапустите системный сервис. Если запись в устройство запрещена, требуется udev-правило для конкретного USB VID/PID принтера.

Для денежного ящика прямой транспорт предпочтительнее CUPS `usb://`: некоторые ESC/POS USB-контроллеры принимают только первое короткое задание после включения, хотя CUPS отмечает последующие задания выполненными. Если ящик открывается после перезапуска принтера, но не открывается повторно, используйте `transportMode: "device"` с `deviceSerial`.

### Локальное открытие денежного ящика по F6

При `cashDrawerHotkeyEnabled: true` Ubuntu-агент регистрирует глобальный `F6` и локально отправляет фиксированную ESC/POS-команду открытия через `receiptPrinter`. CRM и API для хоткея не используются. Удержание клавиши и повторное нажатие во время выполнения не создают очередь команд.

В пользовательской X11-сессии агент использует глобальный X11 hotkey. Системный сервис и Wayland используют Linux input events; установщик добавляет пользователя `avtoforward-agent` в группу `input`. После ручного изменения групп перезапустите сервис. Отключить хоткей можно так:

```json
"cashDrawerHotkeyEnabled": false
```
